...
fn benchmark_topk_add(c: &mut Criterion) {
    ...
    let zipf = ZipfDistribution::new(1000, 1.03).unwrap();
    ...
    for _ in 0..10000 {
        ...
        data.push(key);
        *data_map.entry(key).or_insert(0) += 1;
        topk.add(key);
    }
    ...
    group.measurement_time(std::time::Duration::from_secs(6)); // Increase the measurement time
